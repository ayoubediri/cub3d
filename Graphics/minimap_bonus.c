    ...
    int ix, int iy, int iw, int ih)
{
    ...
    if (sx < ix) sx = ix;
    if (sy < iy) sy = iy;
    if (ex > ix + iw) ex = ix + iw;
    if (ey > iy + ih) ey = iy + ih;
    ...
    int ix, int iy, int iw, int ih)
{
    ...
    if (minx < ix) minx = ix;
    if (miny < iy) miny = iy;
    if (maxx > ix + iw) maxx = ix + iw;
    if (maxy > iy + ih) maxy = iy + ih;
    ...
    int quadrant, int ix, int iy, int iw, int ih)
{
    ...
    if (x0 < ix) x0 = ix;
    if (y0 < iy) y0 = iy;
    if (x1 > ix + iw - 1) x1 = ix + iw - 1;
    if (y1 > iy + ih - 1) y1 = iy + ih - 1;
    ...
    for (int y = y0; y <= y1; ++y)
    {
        for (int x = x0; x <= x1; ++x)
        {
            ...
            if (quadrant == 0 && (dx > 0 || dy > 0)) continue;
            if (quadrant == 1 && (dx < 0 || dy > 0)) continue;
            if (quadrant == 2 && (dx > 0 || dy < 0)) continue;
            if (quadrant == 3 && (dx < 0 || dy < 0)) continue;
    ...
    int ix, int iy, int iw, int ih)
{
    ...
    x0 = cx - r; if (x0 < ix) x0 = ix;
    y0 = cy - r; if (y0 < iy) y0 = iy;
    x1 = cx + r; if (x1 > ix + iw - 1) x1 = ix + iw - 1;
    y1 = cy + r; if (y1 > iy + ih - 1) y1 = iy + ih - 1;
...
void	minimap_render(void)
{
    ...
    while (j < map->height)
    {
        ...
        while (i < map->width)
        {
            ...
            if (is_wall_at(map, parse, i, j))
                draw_wall_border_clipped(mm, map, parse, i, j, px, py, ix, iy, iw, ih);
            else if (parse->map[j] && i < (int)strlen(parse->map[j]) && parse->map[j][i] == '.')
                draw_rect_clipped(px + mm->cell_px / 3, py + mm->cell_px / 3, mm->cell_px / 3, mm->cell_px / 3, mm->col_pellet, ix, iy, iw, ih);
            i++;
        }
        j++;
    }
